...
const Mission = ({ missionName, rewardedPoints, isDone}: IMissionProps) => {
    return (
        <View className='flex flex-row mt-2'>
            <Text className={`w-[70%] text-base ${isDone ? 'line-through' : ''}`}>{missionName}</Text>
...
const DailyMissions = ({ missions}: IMissionsProps) => {
    return (
        <View className='flex flex-col justify-center w-full bg-white mt-2 p-3 rounded-xl'>
            {missions.map((value, index) => <Mission key={index} missionName={value.missionName} rewardedPoints={value.rewardedPoints} isDone={value.isDone} />)}
        </View>
    );
};
