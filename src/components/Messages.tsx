    ...
    return (
        ...
        >
            {message.message !== 'Sent an image' &&
                message.message !== 'Sent you an invitation to a learning room' && (
                    <View
                        className={`mt-4 p-2 rounded-[15px] max-w-[80%] ${isSender ? 'bg-[#1D84C6]' : 'bg-neutral-300'}`}
                    >
                        <Text
                            className={`text-lg font-nunitoSemi text-lg ${isSender ? 'text-white' : 'text-gray-700'}`}
                        >
                            {message.message}
                        </Text>
                    </View>
            ...
            {message.invitation !== null && message.invitation && (
                <View className='bg-gray-300 rounded-[15px] max-w-[80%] p-4 mt-4'>
                    <View className='flex flex-row'>
                        ...
                        <View className='w-[80%]'>
                            <Text className='text-slate-700 font-nunitoSemi text-lg'>
                                You are invited to a learning room! Touch the button to join in!
                            </Text>
                        </View>
                    </View>
                    <TouchableOpacity
