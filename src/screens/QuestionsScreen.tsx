...
import { ArrowLeftIcon, ClockIcon, QueueListIcon } from 'react-native-heroicons/solid';
import { SafeAreaView } from 'react-native-safe-area-context';
import RadioButtonsGroup, { RadioButton, RadioButtonProps, RadioGroup } from 'react-native-radio-buttons-group';
...
const QuestionsScreen = ({ navigation }: QuestionsScreenProps) => {
    ...
    return (
        <SafeAreaView className='mx-3 space-y-4 flex flex-col justify-between h-full'>
            <View>
                <Text className='w-full text-center text-xl text-sky-500 font-semibold'>Assessment Test</Text>
                <View className='flex flex-row justify-between items-center'>
                    ...
                    <View className='flex flex-row justify-between items-center bg-orange-100 p-2 rounded-full'>
                        <ClockIcon color='#fb923c' size={20} />
                ...
                <View className='flex flex-col gap-y-4 mt-4'>
                    <Text className='text-2xl text-gray-700'>1. Do you feel comfortable asking for help when youre stuck?</Text>
                    ...
                    <View>
                        {radGroup.map((value, index) => <TouchableOpacity onPress={() => setSelectedId(value.id)} className='flex justify-center items-start w-full'><Text className={`p-3 w-full ${selectedId === value.id ? 'bg-sky-400 font-semibold text-white' : 'bg-sky-200'} text-base my-2 rounded-2xl`}>{value.label}</Text></TouchableOpacity>)}
                    </View>
                </View>
            </View>
            <View className='flex flex-row justify-between'>
                <TouchableOpacity className='rounded-2xl bg-yellow-400 p-4 mb-5 flex justify-center items-center w-[30%]' onPress={() => navigation.navigate('QuestionsScreen')}>
                ...
                <TouchableOpacity className='p-2 flex justify-center items-center bg-sky-100 mb-5 rounded-2xl'>
                    <QueueListIcon size={40} color='#38bdf8'/>
                </TouchableOpacity>
                <TouchableOpacity className='rounded-2xl bg-yellow-400 p-4 mb-5 flex justify-center items-center w-[30%]' onPress={() => navigation.navigate('QuestionsScreen')}>
