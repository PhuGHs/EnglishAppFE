...
const DetailChat = ({ route, navigation }: DetailChatScreenProps & { route: RouteProp<RootStackParamList, 'DetailChat'> }) => {
    ...
    } = useInput({defaultValue: '', validationFn: () => { return messageValue !== ''; }});
    ...
    const pickImage = async () => {
        try {
            ...
            showToast({ type: 'danger', description: 'Error lauching image library', timeout: 5000 });
        }
    };
            ...
            <View className='px-3 h-[10%]'>
                <View className='flex flex-row justify-between items-center'>
                    ...
                    <TouchableOpacity className='w-fit h-fit flex'>
                        <Image
                            source={profile_picture ? {uri: profile_picture} : require('@asset/images/avatar.jpg')}
                            style={{
                                resizeMode: 'cover',
                                width: 50,
                                height: 50,
